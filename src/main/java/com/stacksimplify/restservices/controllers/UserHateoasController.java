...
public class UserHateoasController {
	...
	public Resources<User> getAllUsers() throws UserNotFoundException{
		...
		for(User user:allUsers){
			...
			Resources<Order> orders=ControllerLinkBuilder
		...
		Link selflinkGetAllUsers=ControllerLinkBuilder.linkTo(this.getClass())
					.withSelfRel();
		Resources<User> finalResources=new Resources<User>(allUsers,selflinkGetAllUsers);
		return finalResources;
	}
	@GetMapping("/{id}")
	public Resource<User> getUserById(@PathVariable(name = "id") @Min(1) Long id){
		try {
